...
# UART1 is used instead of UART0 so system boot logs cannot enter the MCU's
# fixed-length vision protocol. MaixCAM/Pro: A19=UART1_TX, A18=UART1_RX.
VISION_UART_DEVICE = "/dev/ttyS1"
VISION_UART_BAUD = 115200
VISION_FRAME_HEAD = 0x7B
VISION_FRAME_END = 0x7D


def init_vision_uart():
    """Configure MaixCAM UART1 for sending vision results to the MSPM0."""
    err.check_raise(
    ...
    return uart.UART(VISION_UART_DEVICE, VISION_UART_BAUD)


def make_vision_packet(err_center, valid):
    """Build: 7B X_H X_L Y_H Y_L VALID_H VALID_L 7D.

    X/Y are signed target offsets in pixels, encoded by adding 32768. The MCU
    currently uses the area field only as a target-valid flag, so 1 means a
    valid target and 0 means no target.
    """
    if valid:
        x = max(-32768, min(32767, int(err_center[0])))
        y = max(-32768, min(32767, int(err_center[1])))
        area_raw = 1
    else:
        # err_center keeps the previous value after a missed detection; clear
        # coordinates and mark this frame invalid so the MCU does not track it.
        x = 0
        y = 0
        area_raw = 0
...
if __name__ == "__main__":
    ...
    while not app.need_exit():
        ...
        vision_uart.write(make_vision_packet(err_center, update))
        print(err_center)
